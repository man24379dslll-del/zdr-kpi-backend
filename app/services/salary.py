...
ИСКЛЮЧЕНИЕ — группы "ПП" и "Увеличители" (бывшая единая "Выходы на
Пики", разделена на две по прямому запросу заказчика — см.
group_naming.PEAKS_GROUP_RE, supervisor заканчивается на " [Пики-ПП]"
или " [Пики-Увеличители]", признак matches ОБЕ подгруппы одинаково):
у них часовой ставки НЕТ ВООБЩЕ (не 0, а отсутствует как компонент
формулы) — salary = (бонус075 + бонус2) × coefficient, и всё.
work_hours для этих групп игнорируется целиком, даже если колонка
заполнена. Для ВСЕХ остальных групп (включая "Регион УК") формула
обычная, с часовой ставкой.
"""
...
def assign_salary(
    ...
    for r in results:
        ...
        if supervisor and PEAKS_GROUP_RE.search(supervisor):
            # "ПП"/"Увеличители" (бывшая общая "Выходы на Пики"): часовой
            # ставки нет вообще, work_hours не участвует и не проверяется —
            # только бонусы × коэффициент.
