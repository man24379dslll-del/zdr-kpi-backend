...
# Сектор №2 (4 обычных группы супервайзеров, объединённые по запросу
...
# Сектора 2), НЕ вычисляются по паттерну/префиксу, в отличие от Региона
...
SECTOR_2_LK_WEIGHT_EXCEPTIONS_FIO = frozenset({
    "ЗДР ЛК Ремизов А. Г.",
    "ЗДР ЛК Борисенко Н. В.",
    "ЗДР ЛК Шайдуров А. В.",
})

# Сектор №2 — по прямому запросу заказчика диапазон коэффициентов ЛГ уже,
...
# assign_novice_coefficients, где для supervisor из SECTOR_2_SUPERVISORS
# эта таблица подставляется вместо обычной (и число тиров тоже 5, не 10 —
# длина этого списка).
SECTOR_2_TIER_COEFFICIENTS = [1.3, 1.2, 1.1, 1.05, 1.0]


def sector2_zeroes_lk_weight(supervisor: str | None, fio: str | None) -> bool:
    """True, если этому сотруднику нужно урезать ЛК из total_score —
    сотрудник одной из 4 групп Сектора 2, и он НЕ входит в 3 исключения."""
    return bool(supervisor) and supervisor in SECTOR_2_SUPERVISORS and fio not in SECTOR_2_LK_WEIGHT_EXCEPTIONS_FIO


# Сектор №3 (3 обычные группы, отдельный руководитель — Сорокин Никита) —
# точные строки supervisor, найдены по реальным данным, как и для
# Сектора 2. В ОТЛИЧИЕ от Сектора 2: ЛК считается ОБЫЧНО (вес 1.5, без
# исключений/форсированного 1-го места) — по прямому запросу заказчика
# урезание веса ЛК для Сектора 3 не нужно. Общее с Сектором 2 — только
# 2 правила: статус "Новичок" не исключает из официального места (см.
# excel_parsing.py::is_na_row) и суженный диапазон ЛГ (см.
# SECTOR_3_TIER_COEFFICIENTS ниже, те же 5 тиров 1.3...1.0, что у Сектора 2).
SECTOR_3_SUPERVISORS = frozenset({
