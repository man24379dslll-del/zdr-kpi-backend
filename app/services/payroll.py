...
payroll_penalties (штрафы) предполагается уже существующей в Supabase
таблицей, как kpi_uploads/kpi_ratings из старой версии — здесь не
создаётся. Колонки (upload_id, fio, amount) — предположение по аналогии
с остальными таблицами, не сверено с реальной схемой; поправить в
routers/payroll.py, если называются иначе.
