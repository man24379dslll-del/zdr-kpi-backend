...
ВАЖНО про "канал" в среднем: это ПРОМЕЖУТОЧНОЕ ("плоское") место по
каналу, посчитанное apply_category_ranks ДО тира канала (см.
weekly_rating.py и docstring tier_channel.py) — НЕ то итоговое
(тированное) место, которое в конце попадает в r.places["channel"] и
которое видно в таблице "По группам" на сайте. Для человека с ≥10
карточками канала эти два числа могут отличаться (например, если
кто-то с <10 карточками канала обгоняет его по "сумме с контакта" по
всей группе, но не входит в подмножество ">=10 карточек", по которому
считается итоговый ранг). Поэтому вручную свериться по формуле
"место ЛК = размер тира A/Б + среднее из чисел, видимых в таблице" не
получится — это ожидаемое поведение, а не баг. Разорвать эту
неоднозначность нельзя, не сломав саму циклическую зависимость (тир
канала, в свою очередь, использует уже ГОТОВОЕ тированное место ЛК).
"""
