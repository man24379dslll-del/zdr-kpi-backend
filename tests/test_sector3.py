"""
Сектор №3 — 3 обычные группы супервайзеров (Курилова/Бровкова/Басараб).
В ОТЛИЧИЕ от Сектора 2: ЛК считается ОБЫЧНО (вес 1.5, без исключений,
без форсированного 1-го места) — по прямому запросу заказчика урезание
веса ЛК для Сектора 3 не нужно. Общее с Сектором 2 — только 2 правила:
...
from app.services.group_naming import SECTOR_3_SUPERVISORS
...
assert KURILOVA in SECTOR_3_SUPERVISORS
...
def test_sector3_lk_counts_normally_not_zeroed():
    # Контроль: в отличие от Сектора 2, ЛК для Сектора 3 считается как у
    ...
    assert "lk" in a.scores and "lk" in b.scores  # не урезано, как у Сектора 2
...
def test_sector3_novice_gets_real_place_and_tier_not_na():
    # Та же схема, что у Сектора 2: статус "Новичок" сам по себе больше не
...
def test_sector3_novice_with_zero_activity_is_still_na():
    raw = _build_excel_bytes([
        _group_row(KURILOVA),
...
def test_sector3_uses_5_tiers_with_narrow_coefficient_range():
...
def test_normal_group_is_unaffected_by_sector3_logic():
    # Контроль: вне Сектора 3 (и Сектора 2) — всё как раньше: новичок
