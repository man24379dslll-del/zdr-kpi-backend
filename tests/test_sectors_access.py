"""
Руководители Секторов 1 и 2 (аккаунты Бочиус Е.М. / Сорокин Н.) — по
прямому запросу заказчика получают просмотр (НЕ запись) Ведомости ЗП,
ограниченный только группами своего сектора. См.
auth.py::CurrentUser.is_sector1_head/is_sector2_head и
...
def test_is_sector2_head_false_for_sector1_head():
    # Руководитель Сектора 1 — не руководитель Сектора 2, и наоборот.
    user = _user("supervisor", list(SECTOR1_LIST))
    assert user.is_sector2_head is False
    user2 = _user("supervisor", list(SECTOR2_LIST))
    assert user2.is_sector1_head is False


def test_is_sector2_head_false_for_partial_match():
    user = _user("supervisor", [SECTOR2_LIST[0]])
    assert user.is_sector2_head is False


def test_sector_supervisors_for_picks_right_set():
    assert _sector_supervisors_for(_user("supervisor", list(SECTOR1_LIST))) == SECTOR_1_SUPERVISORS
...
def test_restrict_to_sector_filters_rows_by_supervisor():
    result = {
        "rows": [
            ...
            {"fio": "Г", "supervisor": SECTOR2_LIST[0], "sum": 400},
        ],
        "month": 9, "year": 2026,
    }
    restricted_1 = _restrict_to_sector(dict(result), SECTOR_1_SUPERVISORS)
    ...
    restricted_2 = _restrict_to_sector(dict(result), SECTOR_2_SUPERVISORS)
