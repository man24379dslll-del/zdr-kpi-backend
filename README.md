...
- `app/services/tier_channel.py` — **тир канала** (Радио+ТВ / Интернет),
  упрощённый аналог тира ЛК: 2 случая вместо 3 (без подтипов Б/В), порог
  попадания в "тир A" — тот же самый, что у тира ЛК (карточек >= 1 И
  конверсия > 0%). Считается в `weekly_rating.py` ПОСЛЕ тира ЛК —
  использует уже тированное место по ЛК, тир ЛК использует ещё
