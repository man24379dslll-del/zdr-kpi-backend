...
- Группа супервайзера Курбановой делится в `excel_parsing.py` на 2
  ...
  категориям — по общему пулу компании как всегда, а разной у подгрупп
  делает только сама строка `supervisor` — за счёт этого ЛГ
  (`ladder_groups.py`, уже группирует по `supervisor`) естественно
  считается отдельно для каждой подгруппы, без изменений в
  `weekly_rating.py`. Покрыто тестами в `tests/test_weekly_rating.py`.
