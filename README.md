...
- `app/services/ladder_groups.py` — **ЛГ (лестничные группы, тиры 1..10)**,
  ...
  Покрыт тестами (`tests/test_ladder_groups.py`).
- `app/services/salary.py` — **ЗП недели**: (недельная база + бонус075 +
  бонус2) × коэффициент ЛГ ПРОШЛОЙ недели (прошлая неделя ищется внутри
  того же месяца по `period_label`; переход через границу месяца не
  обрабатывается — коэффициент по умолчанию 1.0). Применяется только к
...
3. ~~**ЛГ-коэффициенты** (1.4…0.25 по тирам) и их перенос на следующую
   ...
   + `app/services/salary.py`. Известное ограничение: поиск коэффициента
   "прошлой недели" не обрабатывает переход через границу месяца (period_label
   не содержит год) — в этом случае просто нет найденной прошлой недели
   и коэффициент по умолчанию 1.0
