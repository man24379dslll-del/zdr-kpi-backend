...
- `app/services/ladder_groups.py` — **ЛГ (лестничные группы, тиры 1..10)**,
  точный перенос из старой JS-версии; распределяет коэффициенты 1.4..0.25
  по итоговому месту в рейтинге, отдельно внутри каждого супервайзера.
  Покрыт тестами (`tests/test_ladder_groups.py`). Пока не подключены:
  сама формула ЗП следующей недели и перенос коэффициента между неделями
  (см. п.3 ниже).
- `app/services/weekly_rating.py` — **связывает всё в один пайплайн**:
  ...
  xlsx + `period_label`, категории берёт из `rating_categories`, считает
  рейтинг за неделю, пишет результат в `kpi_uploads`/`kpi_ratings`
...
- `app/services/excel_parsing.py` — разбор реального xlsx (точные
  ...
  отказоустойчивость к отсутствующим "кол-во"-колонкам и выбор канала
  Радио+ТВ/Интернет по большей сумме). Покрыто тестом на реалистичном
  примере (`tests/test_weekly_rating.py`, 10 человек/2 супервайзера/оба
  канала/все тиры ЛК/маппинг в строку kpi_ratings).
...
3. **ЛГ-коэффициенты** (1.4…0.25 по тирам) — распределение по тирам
   перенесено и подключено к недельному расчёту (`ladder_groups.py` +
   `weekly_rating.py` + `POST /ratings/compute`), но их **перенос на
   следующую неделю и подстановка в формулу ЗП** — ещё нет
