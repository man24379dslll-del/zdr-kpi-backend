...
- `app/services/weekly_rating.py` — **связывает всё в один пайплайн**:
  ...
  xlsx, категории берёт из `rating_categories`, возвращает готовый рейтинг
  за неделю (JSON, без записи в БД — таблица под результат из старой
  версии в этом репозитории пока не заведена). Разбор Excel — в
  `app/services/excel_parsing.py`. Покрыто тестом на реалистичном примере
  (`tests/test_weekly_rating.py`, 10 человек/2 супервайзера/все тиры ЛК).
